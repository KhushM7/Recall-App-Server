...
class DatabaseOperations:
    ...
    def mark_today_unreviewed_as_priority(self, user_id: int, review_date: str) -> None:
        """
        Mark unreviewed cards that were due today (i.e., priority 0) as priority and push their next review to tomorrow.
        """
        next_day = (
            (datetime.strptime(review_date, "%Y-%m-%d") + timedelta(days=1))
            .date()
        ...
        # Update unreviewed cards that were due today (priority 0)
        self.conn.execute(
            """
            UPDATE UserPerformance
